...
export default {
	...
	data () {
		return {
			...
			placeList: [
				...
				{name:'西安', geoCoord:[108.91, 32.27]},
				{name:'西安', geoCoord:[108.35, 34.57]},
				{name:'西安', geoCoord:[108.55, 34.77]},
				{name:'西安', geoCoord:[108.85, 34.17]},
				{name:'西安', geoCoord:[108.15, 34.27]},
				{name:'西安', geoCoord:[108.35, 34.87]},
				{name:'西安', geoCoord:[108.91, 34.87]},
				{name:'西安', geoCoord:[107.95, 36.27]},
				{name:'西安', geoCoord:[109.95, 34.27]},
				{name:'西安', geoCoord:[109.35, 34.47]},
				{name:'西安', geoCoord:[109.95, 33.27]},
				{name:'西安', geoCoord:[108.45, 35.27]},
				{name:'西安', geoCoord:[103.95, 36.27]},
				{name:'西安', geoCoord:[103.45, 32.27]},
				{name:'西安', geoCoord:[109.95, 38.27]},    
				...
				{name:'乌鲁木齐', geoCoord:[86.63, 44.77]},
				{name:'乌鲁木齐', geoCoord:[85.67, 43.37]},
				{name:'乌鲁木齐', geoCoord:[86.61, 42.77]},
	...
	methods: {
		handleData(data) {
			if(data.status == '1'){
				this.placeList.push({name:"新增",geoCoord:data.gps});
			}
			if(data.status == '2'){
				this.placeList.forEach((el,index)=>{
					if(JSON.stringify(el.geoCoord) == JSON.stringify(data.gps)){
						this.placeList.splice(index, 1)
						console.warn(JSON.stringify(el.geoCoord)+"----"+JSON.stringify(data.gps)+'--'+index+"---"+this.placeList.length)
					}
				})
			}			
		},
				...
				series : [
					{
						...
						markPoint : {
							...
							data : (()=>{
								...
								var geoCoord
								while(len--) {
									geoCoord = this.placeList[len % this.placeList.length].geoCoord;
									data.push({
										geoCoord : [
											geoCoord[0],
											geoCoord[1]
